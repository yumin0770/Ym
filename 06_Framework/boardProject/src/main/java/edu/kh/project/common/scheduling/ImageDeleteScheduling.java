...
@Component //@Controller,@Service, @Repository 의 부모 어노테이션
			//Bean 등록을 하기겠다고 명시하는 어노테이션
public class ImageDeleteScheduling {
	...
	/*
	 * @Scheduled(fixedDelay = 10000) //ms단위 //일5초->10초 대기 ->일 5초 -> 10초 대기
	 */	
	//@Scheduled(fixedRate = 10000)
	//일(5초) 
	//대기(10초) 동시
	
	//cron="초 분 시 일 월 요일 [년도] 띄어쓰기로 구분
	@Scheduled(cron = "0 0 * * * *")//매 분 0초, 30초 마다
//	@Scheduled(cron = "0 0  * * * *")//매 정시(*시 0분 0초)
	public void test() {
		...
		//서버에 저장된 파일 목록을 조회해서 DB에 저장된 파일 목록과 비교하여 
		//매칭되지 않는 서버 파일을 제거
		
		//1)서버에 저장된 파일 목록 조회
		//->application 객체를 이용해서 "/resources/images/board" 실제 경로를 얻어옴
		String filePath = servletContext.getRealPath("/resources/images/board");
		
		//2) filePath에 저장된 모든 파일 목록 읽어오기
		File path = new File(filePath); //정보를 가져오는 객체 만듦
		...
		//4) 서버에 DB모두 파일 목록이 있을 경우에 비교 시작
			if(!serverImageList.isEmpty()) {
				
				//5)서버 파일 목록을 순차 접근
				for(File server : serverImageList) {
					
					//6) 서버 파일 존재하는 파일이 db이미지 리스트에 없다면 삭제
					/*
					 * String[] temp = server.toString().split("\\"); String s =
					 * temp[temp.length-1];
					 */
					
					/* server.getName() */
					
					//List.indetOf(객체) = 객체가 List에 있으면 해당 인덱스 반환 없으면 -1 반환
				if(dbImageList.indexOf(server.getName()) == -1) {
					//db파일 목록      서버 파일 이림 == -1 (없음)
