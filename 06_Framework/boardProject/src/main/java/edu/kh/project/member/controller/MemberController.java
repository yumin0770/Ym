...
//	@RequestMapping(value="/login", method=RequestMethod.POST)
	public String login(HttpServletRequest req) {
		...
		//매개변수에 적으면 왜 사용 가능할까??
		//Spring Framwork가 제공하는 Argument Resolver (매개변수 해결사)
		//
	...
	public String login(/* @RequestParam("inputEmail") */String inputEmail,
		...
		//****파라미터 name 속성 값과 매개 변수명이 같으면 @RequestParam 생략 가능!****
	...
	public String login(/* @ModelAttribute */ Member inputMember) {
		...
		//파라미터 전달 방법 3 : @MedelAttribute 이용한 방법
		
		// - DTO(또는 VO)와 같이 사용하는 어노테이션
		
		// - 전달 받은 파라미터의 name 속성 값이 
		// 같이 사용되는 DTO의 필드명과 같다면 자동으로 setter를 호출해서 필드에 값을 세팅 
		
		// ***@ModelAttribute 사용 시 주의사항 ***
		// - DTO에 기본 생성자가 필수로 존재해야 한다!!!
		// - DTO에 setter가 필수로 존재해야 한다!
		
		//*** @ModelAttribute 어노테이션은 생략이 가능ㅎ다! ***
