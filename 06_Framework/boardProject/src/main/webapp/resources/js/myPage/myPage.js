...
const deleteImage =  document.getElementById("deleteImage"); //x 태그

let initCheck; //초기 프로필 이미지 상태로 저정하는 변수
            //false == 기본 이미지, true == 이전 업로드 이미지
let deleteCheck = -1; 
//프로필 이미지가 새로 업로드 되거나 삭제되었음을 나타내는 변수
//-1 == 초기값, 0 == 프로필 삭제(x버튼) 1== 새 이미지 업로드

let originalImage;//초기프로필 이미지 파일 경로 저장
...
if(imageInput !=null){//화면에 imageInput이 있을 경우
    ...
    //change 이벤트 : 값이 변했을 때
    //-input type="file",chechk, radio에서 많이 사용
    //text/number 형식 사용 가능
    //-> 이때 input 값 입력 후 포커스를 읽었을 때 이전값과 다르면 change 이벤트 발생
    ...
    imageInput.addEventListener("change", e=>{
        ...
        console.log(e.target.value); //업로도된 파일 경로
        console.log(e.target.files); //업로드된 파일의 정보다 담긴 배열

        const file = e.target.files[0]; //업로드한 파일의 정보다 담긴 객체
        ...
        //매개변수에 작성된 파일을 읽어서 파일을 나타내는 URL로 변환한다
        //파일을 나타내는 URL을 result속성으로 얻어올 수 있게 함.
    ...
    document.getElementById("profileFrm").addEventListener("submit",e=>{
        ...
        //프로필 이미지가 없다가 -> 있다
        if(!initCheck && deleteCheck == 1) flag = false; //기본이미지에서 새 이미지로
        ...
        if(flag){ //flag == true ->제출하면 안 되는 경우
            e.preventDefault(); //form태그의 기본 이벤트 제거
