...
document.getElementById("updateBtn").addEventListener("click",()=>{
  ...
  location.href  //get방식 요청이다
    =location.pathname.replace("board","board2")
    +"/update"
    +location.search
    ///board2/1/2004/update?cp=1 (GET)
});
...
document.getElementById("deleteBtn").addEventListener("click",()=>{
  ...
  if(confirm("정말 삭제 하시겠습니까?")){
    ...
    =location.pathname.replace("board","board2")
    +"/delete"
 ///board2/1/2004/delete (GET)

 //삭제 서비스 호출 성공 시 redirect:/board/{boardCode} 
 //+ RedirectAttributes 이용해서 "삭제 되었습니다" alert 출력

 //삭제 서비스 호출 실패 시 refirect:/board/{boardCode}/{boardNo}
 //+ RedirectAttributes 이용해서 "삭제실패" alert출력
