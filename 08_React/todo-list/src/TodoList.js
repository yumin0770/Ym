import React, { useState, useContext } from 'react'; //react패키지 안에 있다
import { TodoListContext } from './App';

const TodoList = () => {

    const { todoList, setTodoList, loginMember } 
        = useContext(TodoListContext); //전역변수 컨텍스트 쓸건데 TodoListContext쓰겠다 (get)

    const [inputTodo, setInputTodo] = useState(''); //변수 설정 <밑에서 쓰려고 저장함>
    ...
    const handleDeleteTodo = (todoNo, index) =>{
        ...
        .then(result => {
            ...
            const newTodoList = [...todoList]; //깊은 복사 (배열 복사)
    ...
    return (
        <>
            <h1>{loginMember.name}의 To Do List</h1> {/* TodoListContext애서 가져옴 */}
