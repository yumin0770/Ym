...
/*
    CREATE TABLE 테이블명 (
        컬럼명 자료형(크기), (EX)DEPT_CODE, EMP_NAME 같은 것들 
        컬럼명 자료형(크기),
        ...);
*/
...
CREATE TABLE MEMBER(
	...
	MEMBER_NAME VARCHAR2(15), --한글로 2글자 ~ 5글자 (3BYTE)
	MEMBER_SSN  VARCHAR(14),   --990123-1234567 항상 14글자
...
--				  DEFAULT로 작성하면 DEFALUT 오른쪽에 작성된 값에 컬럼 반영됨.
...
INSERT INTO MEMBER
VALUES('MEM03','1Q1W3E4R','박철수','980505-1987654',DEFAULT);
...
INSERT INTO MEMBER2 VALUES ('김길동',0001011324567,01012341234);

SELECT*FROM MEMBER2;
--9001011234567 1012341324

--> NUMBER타입의 컬럼은 값 제일 앞 0을 유지하지 않는 문제가 발생.
...
-- 1. NOT NULL  //NULL 저장 ㄴㄴㄴ//
-- 해당 컬럼에 반드시 값이 기록되어야 하는 경우 사용
-- 삽입/수정시 NULL값을 허용하지 않도록 컬럼레벨에서 제한


CREATE TABLE USER_USED_NN(  --NOT NULL 줄여서'NN'
    USER_NO NUMBER NOT NULL, -->컬럼 레벨 제약 조건 설정   --USER_NO=사용자 번호
    ...
    --> 테이블 레벨 제약조건 설정 //컬럼 다 적고 제약조건을 나중에 적어봄
);
...
INSERT INTO USER_USED_NN
VALUES(NULL, NULL, NULL, NULL, NULL, '010-1234-5678', 
		'hong123@kh.or.kr');
...
--   USER_ID VARCHAR2(20) CONSTRAINT USER_ID_UK UNIQUE,
    ...
    --테이블 레벨 제약 조건
--    , UNIQUE(USER_ID) --제약조건(컬럼명)
    ,CONSTRAINT USER_ID_UK UNIQUE(USER_ID) --이름(0)
...
INSERT INTO USER_USED_UK
VALUES(1, NULL, 'pass01', '홍길동', '남', '010-1234-5678', 
		'hong123@kh.or.kr');
--> 아이디에 NULL 값 삽입 가능.
...
AND UCC.CONSTRAINT_NAME = 'USER_ID_UK'; --USER_ID_UK
...
INSERT INTO USER_USED_UK2
VALUES(2, 'user02', 'pass01', '홍길동', '남', 
		'010-1234-5678', 'hong123@kh.or.kr');
--> USER_ID가 다름 -> 삽입 성공

INSERT INTO USER_USED_UK2
VALUES(2, 'user02', 'pass01', '고길동', '남', 
		'010-1234-5678', 'hong123@kh.or.kr');
--> USER_NAME가 다름 -> 삽입 성공
...
VALUES(1, 'user01', 'pass01', '홍길동', '남', 
		'010-1234-5678', 'hong123@kh.or.kr');
--> 삽입 실패 
-- ORA-00001: 무결성 제약 조건(KH_KYM.USER_ID_NAME_UK)에 위배됩니다
...
-- 3. PRIMARY KEY(기본키) 제약조건 *************중요~!

SELECT*FROM EMPLOYEE;

-- 테이블에서 한 행의 정보를 찾기위해 사용할 컬럼을 의미함
-- 테이블에 대한 식별자(IDENTIFIER) 역할을 함
-- NOT NULL + UNIQUE 제약조건의 의미 --둘이 합쳐진거 처럼되어있음! 의미만 비슷
...
--    USER_NO NUMBER PRIMARY KEY,  --컬럼레벨(제약조건명 X)-
...
--> 기본키가 NULL 이므로 오류
--ORA-01400: NULL을 ("KH_KYM"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다



---------------------------------------

-- PRIMARY KEY 복합키 (테이블 레벨만 가능) --두 컬럼 값을 묶어서함
...
-->ORA-00001: 무결성 제약 조건(KH_KYM.PK_USERNO_USERID)에 위배됩니다
...
-->ORA-01400: NULL을 ("KH_KYM"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
-->복합키로 pk가 설정되어 있어도
--> 결국은 PK ==> NULL 삽입 불가능
ROLLBACK;
----------------------------------------------------------------------------------------------------------------
...
INSERT INTO USER_USED_FK
VALUES(2, 'user02', 'pass02', '이순신', '남', '010-5678-9012',
		'lee123@kh.or.kr', 10);

INSERT INTO USER_USED_FK
VALUES(3, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 
		'yoo123@kh.or.kr', 30);

INSERT INTO USER_USED_FK
VALUES(4, 'user04', 'pass04', '안중근', '남', '010-2222-1111', 
		'Sahn123@kh.or.kr', null);
--> NULL 사용 가능. (참조하는게 없다는 의미- FK는 참조하는거니까)
--> NULL(외부 테이블에 참조할 값이 없다, 참조 X)사용 가능.

SELECT * FROM USER_USED_FK;

INSERT INTO USER_USED_FK
VALUES(5, 'user05', 'pass05', '윤봉길', '남', '010-6666-1234', 
	'yoon123@kh.or.kr', 50);
--> 50이라는 값은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로
 -- 외래키 제약 조건에 위배되어 오류 발생.
-- ORA-02291: 무결성 제약조건(KH_KYM.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
...
DELETE FROM USER_GRADE
WHERE GRADE_CODE = 10;
--ORA-02292: 무결성 제약조건(KH_KYM.GRADE_CODE_FK1)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
CREATE TABLE USER_USED_CHECK(
  ...
  USER_NAME VARCHAR2(30),
  --컬럼 레벨 
  GENDER VARCHAR2(10) CONSTRAINT GENDER_CHECK CHECK (GENDER IN ('남','여')),  --'남''여'자리에 함수X
  PHONE VARCHAR2(30), 
  EMAIL VARCHAR2(50)
);
...
--ORA-02290: 체크 제약조건(KH_KYM.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
	...
	PNO VARCHAR2(20) CONSTRAINT UK_PNOT NOT NULL,
	GENDER VARCHAR2(3) CONSTRAINT CK_GENDER CHECK (GENDER IN('남','여')),
	PHONE VARCHAR2(20),
	ADDRESS VARCHAR2(100),
	STATUS VARCHAR2(3) CONSTRAINT NN_STATUS NOT NULL,
	CONSTRAINT CK_GENDER CHECK (GENDER IN ('Y','N')),
	CONSTRAINT UK_PNOT UNIQUE (PNO)
...
COMMENT ON COLUMN USER_TEST.STATUS   IS '회원번호';

INSERT INTO USER_TEST
VALUES (1, 'mem01', 'abc123', '880122-1234567', '남', '010-1111-9999', '서울시 강남구 역삼동', 'N');
INSERT INTO USER_TEST
VALUES (2, 'mem02', 'abc312', '890222-2234567', '여', '010-2222-9999', '서울시 강남구 삼성동', 'N');
INSERT INTO USER_TEST
VALUES (3, 'mem03', 'abc321', '900322-2234567', '여', '010-3333-9999', '서울시 강남구 청담동', 'Y');
INSERT INTO USER_TEST
VALUES (4, 'mem04', 'efg123', '910422-1234567', '남', '010-4444-9999', '서울시 강남구 도곡동', 'N');
INSERT INTO USER_TEST
VALUES (5, 'mem05', 'efg321', '920522-2234567', '여', '010-5555-9999', '서울시 강남구 대치동', 'N');
...
--3)테이블의 형태(컬럼명, 데이터타입)만 복사 (컬럼 값X)
...
--바꾸다 테이블                 제약조건 추가    조건 이름    어떤 제약 어떤 컬럼
