 ...
 * 
 * -RESULT SET은 0개 이상의 행이 포함될 수 있디.
...
-- 리터럴: 표현되는 값 자체
-- DB에서 리터럴 : 임의로 지정한 값을 기존 테이블에 존재하는 값 사용
--테이블에 없는데 있는 것 처럼
--> 리터럴 표기법: ''(홑따옴표, 기본적으로 문자열 의미)

SELECT EMP_NAME, SALARY, '원'AS 단위 --DA에서 말하는 리터럴
FROM EMPLOYEE;

----------------------------------------------------------

--<DISTINCT> (별개의,따로따로)
--> 조회시 지정된 컬럼에 중복 값을 한 번만 표시할 때 사용

-- EMPLOYEE 테이블에서 현재 사원들이 속해있는 부서코드 종류 조회
...
--EMPLOYEE 테이블에서
--급여가 300만이상 500만 이하인 직원의 모든 정보 조회
...
--컬럼병 BETWEEN A AND B: 컬럼값이 A이상 B이하인 경우
SELECT *
FROM EMPLOYEE 
...
--컬럼명 IN (값1, 값2, 값3,....) : 컬럼 값에 () 안의 값 포함된 행은 TRUE
 ...
 * 
 * '_' (글자 수)
 * - 'A_': A 뒤에 아무거나 한글자만 있는 문자 
 * 		   (AB, A1, AQ, A가)
 * 
 *- '__A' : A 앞에 아무거나 3글자 있는 문자열
 **/
 ...
 -- EMPLOYEE 테이블에서 이름에 '하'가 포홤된 사원의 사번, 이름 조회
...
--> 문제점 : 와일드 카드 문자와(_)와 패턴에 사용된 일반 문자가
-- 		   같은 문자이기 때문에 구분이 안 되는 문제가 발생
...
/*1*/FROM EMPLOYEE 
/*2*/WHERE 연봉 >= 50000000;  
---오류 발행(SELEC절 해석 안된 상태에서 별칭 WHERE절 작성)
