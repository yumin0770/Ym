...
/*  서브쿼리 유형
    ...
    - 다중열 서브쿼리 : 서브쿼리의 SELECT 절에 자열된 항목수가 여러개 일 때 (행 1, 열 N)
...
AND SALARY > ANY(SELECT SALARY
				 FROM EMPLOYEE 
				 JOIN JOB USING(JOB_CODE)
				 WHERE JOB_NAME = '과장'); -- '>' 서브쿼리를 기준으로 가장 작은 값 보다 작은지 비교한다.
				 						  -- '<' 서브쿼리를 기준으로 가장 큰 값 보다 큰지 비교한다.
...
AND SALARY > ALL (SELECT SALARY FROM EMPLOYEE 
			  	  NATURAL JOIN JOB 
			      WHERE JOB_NAME = '차장'); -- 서브쿼리의 어느 값들보다 가장 큰 값을 찾는 것 
...
-- 2) 직급별 평균 급여
SELECT JOB_CODE, TRUNC(AVG(SALARY),-4) -- (-4는 만 단위 밑으로 소수점 버림)
FROM EMPLOYEE 
GROUP BY JOB_CODE;
...
WHERE HIRE_DATE
	=(
		SELECT MIN(HIRE_DATE) FROM EMPLOYEE SUB	
		WHERE (ENT_YN = 'N' AND NVL(SUB.DEPT_CODE,1) =NVL(MAIN.DEPT_CODE,1)) --NULL X
		  --NULL인 경우
	)
ORDER BY HIRE_DATE;
...
/*3*/SELECT EMP_NAME, ROWNUM
/*1*/FROM EMPLOYEE e 
/*2*/WHERE ROWNUM =2 ;  --결과 없음
/*2*/WHERE ROWNUM >=5 ;--결과 없음
...
/*1*/FROM EMPLOYEE
/*2*/WHERE ROWNUM <=5
/*4*/ORDER BY SALARY DESC; --급여 5명을 내림차순 한 것 뿐
-->EMPLOYEE 테이블 위에서 부터 5명을 가지고 급여 내림차순 조회 한 것 뿐! 의도와는 x

--문제 원인: SELECT해석 순서상 ORDER BY가 가장 마지막으로 해석되기 때문에
...
-- 가상의 테이블(VIEW)로 이용할 예정 (행 순서, 컬럼 그래도 이용)
...
--> 조건절에 ROWNUM를 작성하는 경우
-- 반드시 조건 범위 내에 1(1번행)이 반드시 포함되어야 한다.
...
/*0*/ WITH TOP_SALARY AS (SELECT EMP_NAME, SALARY
					 FROM EMPLOYEE 
					 ORDER BY SALARY DESC)
...
--급여를 많이 받는 순서로 조회
SELECT EMP_NAME,SALARY, RANK() OVER(ORDER BY SALARY DESC) 순위  --ONLY SELECT절에서만 쓰인다
FROM EMPLOYEE
WHERE RANK() OVER(ORDER BY SALARY DESC) <=5;  ---안됨 그래서 아래처럼 해야됨
...
SELECT EMP_NAME,SALARY, DENSE_RANK() OVER(ORDER BY SALARY DESC) 순위
	...
	--1.ROWNUM, 2.RANK, DENSE_RANK 많이 씀!!
