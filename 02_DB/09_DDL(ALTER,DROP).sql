...
--> DEPT_ID, LOCATION_ID에만 NOT NULL제약조건 설정됨
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT UK_DEPT_TITLE; --LOCATION
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C007590; --NOT NULL
...
-->새로 추가된 컬럼 LNAME에 기본갑 '한국'이 자동 삽입
...
--한글은 1글자에 3바이트 10바이트보다 더 큰 값 존재하기 때문에 일부 값이 커서 줄일 수 없다함.
--ORA-01441: 일부 값이 너무 커서 열 길이를 줄일 수 없음
-->이미 저장된 값이 수정하는 데이터 타입의 크기보다 크면 수정할 수 없다!
...
--DEPT_COPY의 LNAME 컬럼의 모든 갓을 기본값으로 수정
UPDATE DEPT_COPY
SET LNAME = DEFAULT;
...
--DDL: 테이터 정의 언어(객체를 CREATE,ALTER,DROP)
--DML: 테이터 조작 언어(테이블에 값을 INSERT,UPDATE,DELETE)
--TCL: 트랜잭션 제어 언어(DML 수행 내용을 COMMIT, ROLLBACK, SAVEPOINT)
 
--DBeaver ->TRANCATION(INSERT,UPDATE,DELETE포함)  >DB로 이동
 --(ALTER 사이즈가 달라서 트랜잭션 안에 있는 내용도 다 같이 밀고 DB로 가버림=바로 COMMIT한다)
 --->ROLLBACK 못함. 자동 커밋해서!
