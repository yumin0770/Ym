...
--함수는 SELECT절, WHERE절, ORDER BY절, GROUP BY절, HAVING절 가능
...
-- 길이 미작성 시 시작 위치 부터 끝까지 잘랜서 반환

SELECT SUBSTR('ABCDEF', 3,3) FROM DUAL; -- CDE

SELECT SUBSTR('ABCDEF', 3) FROM DUAL; -- CDE

--EMPLOYEE 테이블에서 사원며과 사원의 이메일 아이디만 조회
-- 아이디 오름차순 정렬
...
-- SYSTIMESTAP : SYSDATE 반환값 + MS 단위 추가
SELECT SYSDATE,SYSTIMESTAMP  FROM DUAL;
--2023-03-07 12:23:01.000,	2023-03-07 12:23:01.051 +0900

--------------------------------------------------------------

--MONTHS_BETWEEN(날짜, 날짜) : 두 날짜의 개월 수 차이를 반환
SELECT '약'|| ROUND(MONTHS_BETWEEN('2023/07/14','2023/02/06'))
		||'개월'훈련기간
FROM DUAL;
...
SELECT EMP_NAME, HIRE_DATE
	,CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE )/12) "근속 개월 수"
...
SELECT 1000000,
	...
	TO_CHAR(1000000, '$9,999,999')  -- $1000000
FROM DUAL;		 
...
/* 날짜 패턴 Y, R의 차이점 */
-- 년도를 짧게 해석하는 경우 
-- 50미만 : Y,R 둘다 앞부분에 현재 세기를(21C==2000년대) 적용한다
-- 50이상 : Y == 현재 세기(2000년대)
--		   R == 이전 세기(1900년대)
...
--NVL2(컬럼명, NULL X인 경우 값, NULL 0인 경우 값)
...
-- DECODE(계산식 | 컬럼명, 조건1, 결과1, 조건2, 결과2,....[, 아무것도 일치 안 할때])
...
SELECT COUNT(DEPT_CODE) FROM EMPLOYEE ; --21 (NULL제외)

SELECT COUNT(*) FROM EMPLOYEE 
WHERE DEPT_CODE IS NOT NULL; --이렇게도 가능하다

--EMPLOYEE 테이블에 존재하는 직급의 개수
