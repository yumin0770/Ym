 ...
 * 2)테이블의 진짜 모습을 감출 수 있다 보안상 유리 -표현하는 것만 표현
 * 
 * **VIEW 사용 시 주의 사항**
 * 1)가상의 테이블(실체X)이기 때문에 ALTER 구문 사용 불가 (컬럼 어쩌고 안 함)
 * 2) VIEW를 이용한 DML(INSERT,UPDATE,DELETE)이 가능한 경우도 있지만
 * 	  제약이 많아 따르기 때문에 조회(SELECT) 용도로 대부분 사용.
 * 
 * **VIEW 작성법**    -옵션이 많다!
 ...
 *   없으면 새로 생성(CREATE말고 REPLACE쓰는 이유)
 * 
 * 2)[FORCE | NOFORCE 옵션:
 * FORCE : 서브쿼리에 사용된 테이블이 존재하지 않아도 강제 뷰 생성
 * NOFORCE (기본값) : 서브쿼리에 사용된 테이블이 존재해야만 뷰 생성 
 * 
 * 3)컬럼 별칭 옵션 : 조회되는 VIEW의 컬럼명을 지정
 * [WITH CHECK OPTION]
 * 
 * 4)WITH CHECK OPTION 옵션: (확인하는 것)
...
-->각자 계정에 CREAT VIEW 권한 부여
GRANT CREATE VIEW TO kh_kym;
--부여 만들다

-->다시 kh계정으로 접속해서 VIEW 생성 구문 수행
...
--저장된 것 처럼 보인다!
--->실제 VIEW에 저장된 것이 아니라
--  VIEW를 생성할 때 사용한 서브쿼리의 테이블(DEPT_COPY2)
-- 값이 삽입되어있다!

--원본 테이블(DEPT_COPY2) 확인
SELECT * FROM DEPT_COPY2;
--> 원본 테이블에 삽입됨을 확인
---> INSERT 구문에 포함되지 않은 DEPT_TITLE 컬럼
-- NULL이 삽입되어 있다!(중요)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
--데이터 무결성: 중복X, NULL X ->신뢰도 높은 데이터
-->대부분의 컬럼에 NOT NULL 제약조건 추가되어 있음
--->NOT NULL 이 설정된 테이블을 이용해서 VIEW를 만들면
--INSERT 가 거의 불가능함
 ...
 * 
 * *******SEQUENCE 왜 사용할까??***********
 * PRIMARY KEY : 테이블 내 각행을 구별하는 식별자 역활
 ...
 * **사용법**
 * 
 * 1)시퀀스명.NEXTVAL(VALUE) :다음 시퀀스 번호를 얻어옴.
 * 						(INCREMENT BY만큼 증가된 수)
 * 						단, 생성 후 처음 호출된 시퀀스인 경우
 * 						START WITH에 작성된 값이 반환됨.
 * 2)시퀀스명.CURRVAL (CURRENT VALUE) :현재 시퀀스 번호를 얻어옴.
...
WHERE TEST_NO = (SELECT MAX(TEST_NO) FROM TB_TEST); ***********************다시 강의 듣기3/22
...
SET TEST_NO = SEQ_TEST_NO.NEXTVAL,
	TEST_NAME = '고길동'|| SEQ_TEST_NO.CURRVAL
WHERE TEST_NO = (SELECT MAX(TEST_NO) FROM TB_TEST); ********************************3/22
 ...
 * 	컬럼에 대하여 생성하는 객체
 * 
 * 인덱스 내부 구조는 B*(BINARY,BALANCED) 트리 형식으로 되어있음.
 * 
 ***INDEX의 장점*
 ...
 *	작업 시 이진 트리 구조에 변형이 일어남
 *  -> DML작업이 빈번한 경우 시스템 부하가 늘어 성능이 저하됨.
 * 
 * -인덱스도 하나의 객체이다 보니 별도 저장공간이 필요(메모리 소비)
 * 
 * -인덱스 생성 시간이 필요함.
 * 셀렉트 많이하는데서 많이 씀 EX)도서관 인덱스, 인스타 피드
 *
 *
 * [작성법]
...
WHERE TEST_ID = 'TEST500000';--54ms/19/38/35

--인덱스 사용 O 조회
SELECT * FROM TB_INDEX_TEST 
WHERE TEST_NO = 500000; --2ms/1/0
...
--인덱스 생성 후 조회
SELECT * FROM TB_INDEX_TEST 
WHERE TEST_ID = 'TEST500000'; --6ms/1/
