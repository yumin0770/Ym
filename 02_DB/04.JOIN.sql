...
--기존 INNER JOIN 결과에
-- JOIN구문 왼쪽에 작성된 EMPLOYEE 테이블의 행 중에서
...
--NATURAL JOIN이 성립되지 않는 경우 -> CROSS 조인의 결과 나옴
SELECT EMP_NAME,DEPT_TITLE
FROM EMPLOYEE 
NATURAL JOIN DEPARTMENT;
--EMPLOYEE  : DEPT_CODE CHAR(2) D1 ~ D9, NULL
--DEPARTMENT: DEPT_ID   CHAR(2) D1 ~ D9, NULL
...
--EMPLOYEE, DEAPRTMENT, LOCATION 테이블 조인
...
--*JOIN 순서 중요!!! *
--> 1 + 2 + 3 왼쪽에서 오른쪽으로
-->앞선 두 테이블의(EMPLOYEE,DEPARTMENT) 조인 후 
...
SELECT EMP_NO, EMP_NAME, JOB_CODE,DEPT_CODE,LOCAL_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID =LOCAL_CODE)
...
SELECT EMP_NAME,JOB_NAME, SALARY, (SALARY*NVL((1+BONUS),1))*12 연봉
FROM  EMPLOYEE E
JOIN JOB USING (JOB_CODE)
