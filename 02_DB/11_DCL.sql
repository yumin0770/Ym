 ...
 * - 사용자(USER)에 DB또는 DB객체에 대한 접근 권한을 
 ...
 * 
 * 2. 사용자 계정
 * 	- 데이터베이스에 대하여 질의, 갱신, 보고서 작성등의 
 *    작업을 수행할 수 있는 계정.
 * 
 *  - 업무 필요한 최소한의 권한만을 가지는 것을 원칙으로 한다.
 *  EX) kh계정, workbook계정 
 * 
 * *권한의 종류 **
 * 1) 시스템 권한 : DB접속, 객체 생성 권한
 * -> 관리자 계정 도는 관리자 권한이 있는 계정
...
2) 객체 권한 : 특정 객체를 조작할 수 있는 권한
	객체: TABLE,VIEW,INDEX,SEQUENCE,USER...ETC
...
--0. SQL 구문 작성 형식은 일반 현식(예전 12C 버전 이전 형식)으로 변경
ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;

--1. 사용자 계정 생성
--CREATE USER 사용자명 IDENTIFIED BY 비밀번호;
CREATE USER kym_sample IDENTIFIED BY "sample1234";
			-->대소문자 구분x		--> 대소문자 구분o

--2.생성된 sample 계정 접속 방법 추가
	--> CREAT SESSION(접속 권한)이 없음
...
ALTER USER kym_sample DEFAULT TABLESPACE SYSTEM
...
/*객체 권한 부여/회수*/
--------------------sample 계정으로 접속--------------------------------
--1.kh계정의 EMPLOYEE 테이블 조호
SELECT * FROM KH_KYM.EMPLOYEE;
...
--시스템 권한 부여: GRANT 권한[,권한,....] ON 객체명 TO 계정명;
...
--시스템 권한 회수: REVOKE 권한[,권한,....] ON 객체명 FROM 계정명;
----------------------------------------------------------
...
GRANT CREATE SEQUENCE,
		...
		CREATE TYPE,
		CREATE TRIGGER,
		CREATE TABLE TO kym_sample;
	
-->ROLE을 이용해서 간단히 부여/회수
GRANT RESOURCE TO kym_sample;
REVOKE RESOURCE FROM kym_sample;


--사용자에게 부여된 시스템 권한 조회
SELECT * FROM DBA_SYS_PRIVS 
WHERE GRANTEE = 'KYM_SAMPLE';
...
--2)CONNET(연결하다, 접촉하다)
--CREATE SESSION(DB접속 권한)하나만을 가지고 있는 ROLE
