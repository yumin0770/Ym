 ...
 * 
 * @WebServlet 어노테이션
 * -> 해당 클래스를 Servlet으로 등록하고 매핑할 주소를 연결하고 지사하는 어노테이션
...
public class ExampleController2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//HttpServketRequest : 요청 데이터를 담고있는 객체
		String orderer = req.getParameter("orderer");
		String coffee = req.getParameter("coffee");
		String type = req.getParameter("type");

		//name 속성 값이 같은 파라미터가 여러개인 경우
		//String[]로 한번에 반환받는 getParameterValues("name") 사용
		...
		out.print("<head>  <tilte> "+orderer+" 님의 주문 결과 </title></head>");
		
		out.print("<body>");

		out.print("<h3>주문자명:" +orderer+ "</h3>");
