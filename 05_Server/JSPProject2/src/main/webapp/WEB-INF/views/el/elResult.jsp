...
<body>
 ...
 <%--         address(JSP표현식) : <%= request.getAttribute("address")%> <br>  --%>
        ...
        score : ${score}<br> 
        strList: ${strList}<br> <%-- list 담아서 가져옴 --%>
        book  : ${book} <br> <%-- 객체 담아서 가져옴 --%>
    ...
    strList[3] : ${strList[3]} <br>(3번 인덱스 없음)      <%-- null이라 판단되어 빈칸 --%>   

    <br><br>
   <%--      book의 title필드(JSP표현식) : 
      <%= ( (Book)request.getAttribute("book") ).getTitle() %> <%-- 타입 오브젝트임 --%>
    ...
    <h4>empty 연산자</h4>
     ...
     ${empty list1} / ${empty list2}  <%-- 이거 비어있는거 맞아? empty true! --%>
