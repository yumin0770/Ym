...
<body>
    ...
    <pre>
    JSP의 표현식  <%-- <%= %> expression --%>
    조금 더 효율적이고 간단히 작성 할 수 있도록 고안된 언어
    (JSP에 내장되어 있음)

    화면(HTML)에 출력하고자하는 자바 코드를 

    <%-- \$로 작성하면 화면에 $로 보임 --%>
    \${ key } 형식으로 작성하면 해당위치에 알맞은 value가 출력됨
    </pre>

    <h3>EL의 특징 1번 : get이라는 단어를 사용하지 않는다.</h3>
    <%-- getName -> Name이라고만 쓴다 get뺌 --%>
    ...
    테스트1(JSP표현식) : <%= request.getParameter("num")%>
    <br>
    테스트 2(EL) : ${param.num == '123'} <%--오류를 발생 안 시키려고 null을 빈칸으로 처리한다. 빈칸==123-> false --%>
