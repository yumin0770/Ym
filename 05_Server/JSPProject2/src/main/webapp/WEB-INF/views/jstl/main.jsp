...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
 ...
 <%-- 
        ...
        uri(Uniform Resource Indefirier,통합 자원 식별자) : 
        -> 자원을 식별하는 고유 문자열 
...
<body><%-- 상대경로 절대경로는 / --%>
        ...
        <a href="loop">JSTL을 이용한 반복문(forEach,while)</a>
      ...
      <h1>STL(jsp Standard Tag Library)JSP표준 태그 라이브러리</h1>
    <pre>
        JSP에서 자주 사용되거나 공통적으로 사용되는 JAVA코드(if,for,변수선언,형변환)
        
        스크립틀릿 html 태그 형식을 태그화하며 표준으로 제공하는 라이브러리
    ...
    <ol>
        <li>
        <a href="https://tomcat.apache.org/download-taglibs.cgi"></a><br>
        Standard-1.2.5 -> Jar File -> Impl, Spec, El 다운로드
        </li>
            
        <li>
         /webapp/WEB-INF/lib 폴더에 라이브러리(.jar)추가
        </li>
      
        <li>
            JSTL라이브러리를 사용하고자 하는 JSP파일 상단에 
    ...
    <pre>
        말이 변수언어, 사실은 세팅하는 것으로
        -변수 선언(지정된 scope에 속성 세팅)
        ...
        3)scope : page, request, session, application 중 하나 지정된
                (기본값 page)
    </pre>

    <%
    //스크립틀릿으로page scope에 속성 세팅하는 방법
    pageContext.setAttribute("num1",10);
    %>
    <%-- JSTL(c:set)으로 page scope에 속상세팅하는 방법 --%>
...
<%--    <c:remove var="num2" scope="request" /> --%>
    ...
    <%-- 모든 scope에서 제거  --%>
   <c:remove var="num2"/>
    requst num2 제거 확인 : ${requstScope.num2}
    <br>
