...
<body>
    ...
    <pre>
        Servelt/JSP에는 4종류의 범위를 갖는 내장객체가 존재
        -> 각 종류마다 영향을 끼치는 범위가 다름

        1. page (pageContext) : 현재 페이지(한 페이지)
            -> 현재 Servlet / JSP에서만 사용 가능
        
        2.request : 요청 받은 페이지(Servlet/JSP)와 이를 위임 받은 페이지
                    (Servlet/JSP)에서 사용 가능(최소 2페이지 이상)

        3.session : 현재 사이트 접속한 브라우저당 1개씩 생성
                    브라우저가 종료되거나, session 만료될 때 까지 유효  <%-- 로그인시 --%>
    ...
    <ul>
        <li>
             ...
             <% //내장된 객체 pageContext :page scope객체
        ...
        <li>
            ...
            appValue :  ${applicationScope.addValue} <%-- 10000 --%>       
        </li>
    ...
    <h3>
        <a href="scopeCheck">내장 객체별 생명 주기 확인하기</a>
    </h3>
