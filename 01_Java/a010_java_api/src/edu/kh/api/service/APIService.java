...
public class APIService {
	 ...
	 */
	public boolean addStudents(int grade,int classRoom, int number, String name) {
		
		int index = 0; //비어있는 index 저장
		for(Student s : studentList ) {
			
			if(s == null)  break;  //s==null > == 반복하지마~
	 ...
	 * @return true/false
	 */
	public boolean addStudents2(int grade,int classRoom, int number, String name) {
		
		// 1.equals()를 이용한 비교
		
		// 1) 전달 받은 값으로 임시학생 객체를 생성
		...
		for(Student s : studentList) {
			...
			//s가 참조하는 학생과
			// temp가 참조하는 학생이 필드값이 같을 경우 (동등한 경우)
			//if(s.eaquals(temp)) return false;
			
			System.out.println(s.hashCode());
			System.out.println(temp.hashCode());
			System.out.println("---------------------");
			
			//해시코드 같다.== 필드가 같다== 중복되는 학생이다
	...
	public Student[] selectName(String input) {
		...
		// String.split([구분자]) : 문자열 구분자로 기준으로 나누어  String[]형태로 반환
...
//		String[] names = input.split("/");
		...
		int index = 0; // result 배열에 저장될 위치를 저징할 변수
