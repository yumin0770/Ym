...
import edu.kh.objectarray.service.*;
...
public class StudentView {
	...
	private void addStudent() {
		...
		//학생 추가 서비스 호출
		boolean result = service.addStudent(grade, clasRoom, number, name);
	...
	private void selectAll() {
		...
		// Student가 참조하는 객체의 필드 중 studentArr를 반환 받아
		//각 배열 요소를 화면에 출력
		
		
		// ***********************************************************
		// 매개변수, 반환값으로 참조변수가 상용될 경우
	...
	//Student 객체가 가지고있는 toString() 메서드를 호출해서
		...
		// -> 참조 하고있는 객체가 없는 (null) 참조변수를 이용해서
...
/** 3. 학생 정보 조회 (인덱스)*/
	...
	//반환 받은 index번째 요소가 null이 아닐경우(학생 정보가 있을 경우)
