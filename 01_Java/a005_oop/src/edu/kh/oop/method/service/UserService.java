...
public class UserService {
	...
	//1.회원가입
	public User signUp(String userId, String userPw, String userPwConfirm,String userName,
			 char userGender) {
		...
		User user = null; //아무것도 참조하지 않음 (비어있다는 뜻은 아님)
	
		//비밀번호, 비밀번호 확인이 일치할 경우
		if(userPw.equals(userPwConfirm) ) {
		//	String 참조형 비교시 equals
	...
	public void login(String userId, String userPw, User signUpUser) {
		...
		if(idCheck && pwCheck) {
			...
			//UserView에 있는 클래스 변수(static)선언된 loingUser에 
			//가입한회원의 정보를 가지고 있는 객체의 주소를 대입
			// 클래스명(UserView).필드명.
 ...
 //5. 회원 정보 수정
	public boolean userUpdate(String userName, char userGender, String userPw) {
		...
		if( !UserView.loginUser.getUserPw().equals(userPw)     ) {
			//논리 부정 연산자 사용
			return false; //호출한 곳으로 false와 같이 돌아간다.
		}
		 //비밀번호가 일치할 때
