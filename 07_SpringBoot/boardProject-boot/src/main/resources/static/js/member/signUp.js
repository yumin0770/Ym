 ...
 *** 정규 표현식의 메타 문자***
    ...
    */  
        ...
        //JS 객체 : {"K":Y,"K":Y,"K":Y,"K":Y,} (Map 형식) 자스객체 모양 v에 함수도 들어감. 메서드가 됨
        // 특징 
      ...
      const checkObj = {
        ...
        memberEmail.addEventListener("input",()=>{
            ...
            if(memberEmail.value.trim().length == 0){
                memberEmail.value = ""; //띄어쓰기 꽉차있지 못 하게 없애버림
            ...
            if( regEx.test(memberEmail.value) ){ //유효한 경우
               ...
               //get방식 ajax 요청 (파라미터는 쿼리스트링으로......)
               fetch('/dupCheck/email?email=' + memberEmail.value )

               .then(response => response.text()) //응답객체 -> 파싱(parsing, 테이터 형태 변환)
               ...
               .then(count => {
                ...
                if(count == 0){
                    ...
                    emailMessage.classList.add("error"); //.erro 스타일 적용
               ...
               .catch(err => console.log(err)) //예외 처리
                ...
                emailMessage.classList.remove("comfirm"); //.confirm 스타일 제거
...
memberPw.addEventListener("input",()=>{
    ...
    if(regEx.test(memberPw.value)){
       ...
       if(memberPwConfirm.value.trim().length == 0){ 
        ...
        pwMessage.innerText = "유효한 비밀번호 형식이 아닙니다."
        pwMessage.classList.add("error");
        pwMessage.classList.remove("confirm");
...
memberNickname.addEventListener("input",()=>{
    ...
    if(memberNickname.value.trim().length == ''){
        ...
        memberNickname.value = ""; //빈칸 입력 못하게 해라~
        return;
    } //정규 표현식 유효성 검사
...
checkAuthKeyBtn.addEventListener("click", function(){
    ...
    if(authMin > 0 || authSec > 0){ // 시간 제한이 지나지 않은 경우에만 인증번호 검사 진행
        ...
        document.getElementById("signUpFrm").addEventListener("submit", e=>{
            ...
            //checkObj에 모든 value가 trune인지 검사
            //(배열용 for문)
            //for ...of : 향상된 for문
            ...
            //(객체용 for문)
            //**for ...in 구문.. */
            //->JS 객체가 가지고 있는 key를 순서대로 하나씩 거내는 반복문

            for(let key in checkObj){

                if(!checkObj[key]){ //각 key에 대한 value(true,false)를 얻어 flase인 경우  
                                     // ==유요하지 않다

                switch(key){
                    case "memberEmail":
                        alert("이메일가 유효하지 않습니다"); break;
                    ...
                    //key를 input의 id가 똑같이 설정했음   
                    document.getElementById(key).focus();       

            
                    e.preventDefault();//f개,택그 기본 이벤트 제거
                    return;
            }       
